...
The earlier possible monthly supporter product is superseded by `paid-access.md`: monthly, annual, and lifetime options unlock all current and future features explicitly designated as paid. Premium Support is the current planned benefit; themes are free. Do not create another supporter subscription or attach consumable tips to the `aligner_tray_tracker_pro` entitlement.
