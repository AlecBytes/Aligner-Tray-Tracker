...
Source: #38, amended 2026-09-14. This is the authoritative cross-feature purchase specification. Themes are free and are governed by `themes.md`; they are not part of paid access. This document specifies planned behavior, not completed billing setup.
...
Core tracking, corrections, statistics, treatment plans/history, notifications, themes, and access/export of personal data remain free. This decision does not reclassify existing free features or authorize implementation of future features. Consumable tips support development but grant no access and must never be attached to the paid entitlement.

The currently planned paid benefit is Premium Support, providing priority email support and more personalized troubleshooting while standard support remains available to free users. Other features become paid only through a separate explicit product decision. Do not advertise unspecified future paid features as already available.
...
- Before integration, inspect existing dashboard configuration. If old `themes` products or entitlements already have purchasers, preserve the purchase records and map any promised Pro access deliberately; do not delete or detach purchased products blindly. No theme requires an entitlement in the app.
...
A fresh installation without usable cached access resolves asynchronously and can restore purchases using the same App Store account. Treatment backups and theme preferences never grant purchase access. Future paid feature specs must define safe access-loss behavior without deleting or trapping personal data.
