...
export function AppThemeProvider({ children }: PropsWithChildren) {
  const db = useSQLiteContext(); const [savedThemeKey, setSavedThemeKey] = useState<ThemeKey>('default'); const savedRef = useRef<ThemeKey>('default'); const revision = useRef(0); const queue = useRef(Promise.resolve());
  ...
  const value = useMemo(() => ({ savedThemeKey, effectiveThemeKey: savedThemeKey, selectTheme, reloadTheme }), [reloadTheme, savedThemeKey, selectTheme]);
