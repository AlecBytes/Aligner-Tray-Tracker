...
export function PremiumScreen() {
  ...
  async function buy(item: PaidAccessPackage) { if (busy) return; setBusy(item.id); setError(null); setMessage(null); try { const outcome = await purchase(item.id); if (outcome === 'cancelled') setMessage('Purchase cancelled. You were not charged.'); else if (outcome === 'pending') setMessage('Purchase approval is pending. Premium features will unlock after access is confirmed.'); else if (!pendingTheme) setMessage('Premium access is active.'); } catch { setError('The purchase could not be completed. Please try again.'); } finally { setBusy(null); } }
  ...
  return <Host seedColor={theme.primary} style={{ flex: 1 }}><Form>
    <Section title="Premium"><Text>Unlock all currently available color themes, Premium Support, and paid features as they are released. Core tracking remains free.</Text></Section>
